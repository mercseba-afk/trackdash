...
export const marketScanRuns = pgTable(
  ...
  (table) => [
    index("idx_market_scan_runs_started_at").on(table.startedAt.desc()),
...
export const marketCandidates = pgTable(
  "market_candidates",
  {
    ...
    resolvedReleaseId: uuid("resolved_release_id").references(() => productReleases.id, { onDelete: "restrict" }),
  ...
  (table) => [
    ...
    unique("market_candidates_identity_unique").on(table.id, table.sourceId, table.resolvedReleaseId),
    index("idx_market_candidates_release").on(table.resolvedReleaseId, table.decision),
    index("idx_market_candidates_review").on(table.decision, table.needsRevalidation, table.lastObservedAt.desc()),
...
export const pricePoints = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      name: "price_points_candidate_identity_fk",
    })
      .onUpdate("cascade")
      .onDelete("restrict"),
  ...
  (table) => [
    ...
    check(
      ...
      sql`(${table.value} is null or ${table.value} >= 0)
        and (${table.low} is null or ${table.low} >= 0)
        and (${table.high} is null or ${table.high} >= 0)
        and (${table.median} is null or ${table.median} >= 0)
