...
-- in place (same ids, no INSERT/DELETE): Neo-Tridagger ZMC's product-level
-- originalYear (1998->1996) and both releaseSeedKey 1/2 rows (originalYear
-- fix + releaseDate add; item 95450->false-Premium-2016 replaced with the
-- real Carbon Special). Full audit trail, Tamiya+RCJaz cross-check per
-- release, in docs/CATALOG_AUDIT.md's Catalog Expansion Wave 1 section.
--
-- Updated post-approval (same migration, not superseded by 0017): Avante
-- Mk.III's Azure Clear Special (item 95464) official page was found, on
-- review, to explicitly state BOTH an initial release (2010-09, already
-- recorded on releaseSeedKey "6") AND a distinct current on-sale date
-- (2023-11-11) -- the same dual-date pattern already modeled for
-- Neo-Tridagger ZMC Carbon Special (95508). releaseSeedKey "7" was added
-- to record that 2023 Reissue, consistent with the existing convention.
--
-- No image rows here (see 0016). Deterministic (all ids stableUuid()-derived
-- from immutable seed keys), idempotent-safe (INSERT ... ON CONFLICT (id) DO
-- NOTHING for new rows, plain UPDATE by primary key for corrections).
--
-- Not applied to Supabase in this pass.

insert into products (id, seed_key, item_number, code, name, japanese_name, series, chassis, original_release_year, rarity, description, canonical_release_id) values
  ('1acf7850-c8a9-5627-b104-54db6e235ba2', 'dash-x1-proto-emperor', '18074', NULL, 'DASH-X1 Proto-Emperor', 'ダッシュX1・原始皇帝（プロトエンペラー）', 'Dash! Yonkuro', 'Super II', 2013, 'Rare', 'Jin Kidoin''s machine, Yonkuro Hinomaru''s rival in Dash! Yonkuro. Later superseded in-story by Proto-Emperor ZX -- a distinct machine from that later Product in this catalog.', '0226dfa5-5e29-5557-b134-ddbad7682e28'),
  ('de719716-e50a-5811-b99d-18bbb153b166', 'avante-mk3', '18626', NULL, 'Avante Mk.III', 'アバンテMk.III', 'Avante', 'MS', 2008, 'Uncommon', 'The third-generation Avante PRO racer, launched in Azure and Nero colourways on the new MS chassis. A long-running special-edition family across MS and MA chassis variants.', '497455cb-838d-5430-97dd-ae0be52e69e4')
on conflict (id) do nothing;

insert into product_releases (id, product_id, item_number, edition_name, release_type, edition_type, release_year, release_date, chassis, color, rarity, verification_status) values
  ('d9b9392b-d149-52a2-b862-0eafc66af7ef', '9793fbe8-dcf7-51c9-9f45-185194a0bc92', '95508', 'Neo-Tridagger ZMC Carbon Special (2023 Reissue)', 'Reissue', 'reissue', 2023, '2023-08-12', 'Super II', 'Black', NULL, 'verified'),
  ('0226dfa5-5e29-5557-b134-ddbad7682e28', '1acf7850-c8a9-5627-b104-54db6e235ba2', '18074', 'Dash-X1 Proto-Emperor Premium', 'Premium', 'original', 2013, '2013-01-12', 'Super II', 'Violet', NULL, 'verified'),
  ('6168c423-9f3e-5495-9a1d-06185ea7fa34', '1acf7850-c8a9-5627-b104-54db6e235ba2', '95450', 'Dash-X1 Proto-Emperor Premium Black Special', 'Color Special', 'color_special', 2019, '2019-01-12', 'Super II', 'Black', NULL, 'verified'),
  ('497455cb-838d-5430-97dd-ae0be52e69e4', 'de719716-e50a-5811-b99d-18bbb153b166', '18626', 'Avante Mk.III Azure', 'Original', 'original', 2008, '2008-09-06', 'MS', 'Light Blue', NULL, 'verified'),
  ('86763fe4-bfc0-551e-8541-c3fc9c2442b7', 'de719716-e50a-5811-b99d-18bbb153b166', '18627', 'Avante Mk.III Nero', 'Original', 'other', 2008, '2008-09-27', 'MS', 'Black', NULL, 'verified'),
  ('68be3b41-30a7-55be-8cd9-f741193ce595', 'de719716-e50a-5811-b99d-18bbb153b166', '95087', 'Avante Mk.III Japan Cup 2015 Limited Edition', 'Japan Cup Edition', 'japan_cup', 2015, '2015-07-11', 'MA', 'Magenta', NULL, 'verified'),
  ('3ba49f54-21c9-532d-a34a-7b9e38668a6d', 'de719716-e50a-5811-b99d-18bbb153b166', '95425', 'Avante Mk.III Red Special', 'Color Special', 'color_special', 2018, '2018-12-01', 'MS', 'Red', NULL, 'verified'),
  ('e31c9f48-a776-564a-a496-63771e4a4f9d', 'de719716-e50a-5811-b99d-18bbb153b166', '95469', 'Avante Mk.III White Special', 'Color Special', 'color_special', 2019, '2019-03-23', 'MS', 'White', NULL, 'verified'),
  ('9d9d9015-81e5-5388-9d36-3e8b3b223b54', 'de719716-e50a-5811-b99d-18bbb153b166', '95464', 'Avante Mk.III Azure Clear Special (Polycarbonate Body)', 'Clear Body', 'color_special', 2010, NULL, 'MS', 'Clear/Azure', NULL, 'verified'),
  -- Consistency fix (post-Wave-1 review): the same item's official page
  -- also explicitly states a distinct current on-sale date, 2023-11-11
  -- -- same dual-date pattern as Neo-Tridagger ZMC Carbon Special.
  ('cc1fb7fa-67db-5303-9514-80b9705fe732', 'de719716-e50a-5811-b99d-18bbb153b166', '95464', 'Avante Mk.III Azure Clear Special (2023 Reissue)', 'Reissue', 'reissue', 2023, '2023-11-11', 'MS', 'Clear/Azure', NULL, 'verified')
on conflict (id) do nothing;

update products
set original_release_year = 1996
where id = '9793fbe8-dcf7-51c9-9f45-185194a0bc92';
