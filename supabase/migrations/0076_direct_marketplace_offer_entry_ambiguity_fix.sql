-- Final correction for direct offer entry: qualify marketplace_offers columns
-- because the function returns columns named conversation_id / offer_id.
...
begin
  ...
  if v_currency not in ('EUR', 'USD', 'JPY', 'GBP') then raise exception 'Unsupported currency'; end if;
  ...
  for update;
  if not found then raise exception 'This item is no longer open to offers'; end if;
  if v_share.user_id = v_uid then raise exception 'You cannot make an offer on your own item'; end if;
  ...
  ) then raise exception 'Messaging is unavailable between these collectors'; end if;
  ...
  select p.username into v_requester_username from public.profiles p where p.id = v_uid;
  select cp.username into v_owner_username from public.collector_profiles cp where cp.user_id = v_share.user_id;
  if v_requester_username is null then raise exception 'Your collector profile is unavailable'; end if;
  if v_owner_username is null then raise exception 'Seller collector profile is unavailable'; end if;
  ...
  if found then
    ...
    if v_conversation.status = 'pending' then
      update public.conversations set status = 'accepted' where id = v_conversation_id;
    else
      update public.conversations set updated_at = now() where id = v_conversation_id;
    end if;
  else
    insert into public.conversations(
      collection_share_id, product_id, release_id, owner_id, requester_id,
      owner_username, requester_username, request_message, status, responded_at
    ) values (
      v_share.id, v_share.product_id, v_share.release_id, v_share.user_id, v_uid,
      v_owner_username, v_requester_username, v_request_message, 'accepted', now()
    ) returning id into v_conversation_id;
  end if;

  update public.marketplace_offers as mo
  set status = 'superseded', responded_at = now(), updated_at = now()
  where mo.conversation_id = v_conversation_id and mo.status = 'pending';
