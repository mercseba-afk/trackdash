...
import { ArrowLeft, Ban, Check, ChevronRight, Loader2, MessageCircle, Send, Unlock, X } from "lucide-react"
import {
...
export function MessagesScreenV2() {
  const { user } = useStore()
  ...
  React.useEffect(() => {
    ...
    getMyConversationsAction()
      .then((rows) => {
        if (cancelled) return
        setConversations(rows)
        setSelectedId(requestedConversationId && rows.some((row) => row.id === requestedConversationId) ? requestedConversationId : null)
      })
      .finally(() => {
  ...
  React.useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") void refreshVisibleConversation()
      })
      ...
      if (!result.ok) {
        ...
        toast.error(
          result.reason === "blocked"
            ? it
              ? "I messaggi sono disattivati perché uno dei due collezionisti ha bloccato l'altro."
              : "Messaging is disabled because one collector has blocked the other."
            : it
              ? "Questa conversazione non è aperta ai messaggi."
  ...
  if (loading) {
    return (
      <div className="flex min-h-64 items-center justify-center gap-2 text-sm text-muted-foreground">
        <Loader2 className="size-4 animate-spin" /> {it ? "Caricamento messaggi…" : "Loading messages…"}
      </div>
    )
  }
  ...
  if (!selected) {
    return (
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-5">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{t("messages.title")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">{it ? "Seleziona una conversazione per aprire la chat." : "Select a conversation to open the chat."}</p>
        </div>
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-4">
      <Card className="overflow-hidden py-0">
        <CardHeader className="border-b bg-muted/20 px-3 py-3 sm:px-4">
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" className="size-10 shrink-0 rounded-full" onClick={closeConversation} aria-label={it ? "Torna alle conversazioni" : "Back to conversations"}>
              <ArrowLeft className="size-5" />
            </Button>
            <span className="grid size-10 shrink-0 place-items-center rounded-full bg-brand/10 font-semibold uppercase text-brand">
              {selected.otherUsername.slice(0, 1)}
            </span>
            <div className="min-w-0 flex-1">
            ...
            {selected.blockedByMe ? (
              <Button size="sm" variant="outline" className="shrink-0 gap-1" disabled={busy} onClick={async () => {
                setBusy(true)
                try {
              ...
              }}><Unlock className="size-3.5" /> <span className="hidden sm:inline">{it ? "Sblocca" : "Unblock"}</span></Button>
            ) : (
              <Button size="sm" variant="ghost" className="shrink-0 text-muted-foreground hover:text-destructive" disabled={busy} onClick={async () => {
              ...
              }}><Ban className="size-4" /><span className="hidden sm:inline">{it ? "Blocca" : "Block"}</span></Button>
            )}
          </div>
        </CardHeader>

        <CardContent className="flex flex-col gap-3 p-3 sm:p-4">
          <div className="sm:hidden">{statusBadge(selected.status, it)}</div>

          {!directOfferConversation ? (
            <div className="rounded-xl border bg-muted/25 px-3 py-2.5">
              <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{it ? "Messaggio iniziale" : "Initial message"}</p>
              <p className="mt-1 text-sm leading-relaxed">{selected.requestMessage}</p>
            </div>
          ) : null}
          ...
          {selected.status === "pending" ? selected.isOwner ? (
            <div className="flex flex-col gap-3 rounded-xl border bg-muted/20 p-4">
              {!selected.offerStillOpen ? (
                <p className="text-sm text-muted-foreground">{it ? "Questo modello non è più aperto a offerte, quindi la richiesta non può essere accettata. Puoi comunque rifiutarla." : "This item is no longer Open to offers, so the pending request cannot be accepted. You can still decline it."}</p>
              ) : selected.blockedByMe || selected.blockedByThem ? (
                <p className="text-sm text-muted-foreground">{it ? "La richiesta non può essere accettata finché i messaggi tra questi collezionisti sono bloccati." : "This request cannot be accepted while messaging is blocked between these collectors."}</p>
              ) : null}
            ...
            <p className="rounded-xl border bg-muted/20 p-4 text-sm text-muted-foreground">{it ? "In attesa che il proprietario accetti o rifiuti la richiesta." : "Waiting for the owner to accept or decline your request."}</p>
          ) : null}

          {selected.status === "declined" ? (
            <p className="rounded-xl border bg-muted/20 p-4 text-sm text-muted-foreground">{it ? "La richiesta è stata rifiutata. Nessuna chat è stata aperta." : "This request was declined. No chat was opened."}</p>
          ) : null}
            ...
            <>
              <details className="group rounded-xl border bg-muted/15">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-medium">
                  <span>{it ? "Offerta e compravendita" : "Offer and transaction"}</span>
              ...
              <div className="flex min-h-[420px] max-h-[62vh] flex-col gap-3 overflow-y-auto rounded-2xl border bg-muted/15 p-3 sm:p-4">
                {loadingMessages ? (
                  ...
                  <div className="m-auto flex max-w-sm flex-col items-center gap-2 text-center text-muted-foreground">
                    ...
                    <p className="text-sm leading-relaxed">{it ? "Scrivetevi qui per accordarvi sui dettagli. Offerte e conferme restano nella sezione dedicata sopra." : "Use this chat to agree on details. Offers and confirmations stay in the dedicated section above."}</p>
                  </div>
                ) : messages.map((message) => {
                  const mine = message.senderId === user?.id
                  return (
                    <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[84%] rounded-2xl px-4 py-3 shadow-sm sm:max-w-[72%] ${mine ? "rounded-br-md bg-brand text-brand-foreground" : "rounded-bl-md border bg-card text-foreground"}`}>
