  ...
  return (
    <Link href={href} className="group flex items-center gap-3 rounded-xl px-2.5 py-2.5 transition-colors hover:bg-[#f4f8fd]">
      ...
      <div className="max-w-40 text-right">
        ...
        {row.signal.startingItemPriceEUR != null ? <p className="text-[11px] text-muted-foreground">{it ? "Osservato" : "Observed"} {formatMoney(row.signal.startingItemPriceEUR)}</p> : null}
      </div>
    </Link>
  )
}
