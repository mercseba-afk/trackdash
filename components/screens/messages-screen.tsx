...
function statusBadge(status: Conversation["status"], it: boolean) {
  if (status === "accepted") return <Badge className="bg-brand/15 text-brand">{it ? "Accettata" : "Accepted"}</Badge>
...
export function MessagesScreen() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div><h1 className="text-2xl font-semibold tracking-tight">{t("messages.title")}</h1><p className="text-sm text-muted-foreground">{it ? "La chat si apre solo dopo che il proprietario accetta la richiesta." : "A chat opens only after the owner accepts the request."}</p></div>
