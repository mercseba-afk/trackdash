  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-3">
        {sorted.map((e) => <WishlistRow key={e.item.id} entry={e} onRemove={async () => { try { await removeFromWishlist(e.item.id); toast.success(it ? `${e.product.name} rimosso dai desideri` : `Removed ${e.product.name} from wishlist`) } catch (error) { toast.error(error instanceof Error ? error.message : it ? "Impossibile rimuovere questo elemento" : "Couldn't remove this item") } }} onAcquire={async () => { try { await moveWishlistToCollection(e.item.id, { condition: "New / Opened", acquisitionDate: new Date().toISOString(), acquisitionPrice: e.item.targetPrice ?? e.estimate.value, acquisitionCurrency: "EUR" }); toast.success(it ? "Spostato nella collezione" : "Moved to collection", { description: e.product.name }) } catch (error) { toast.error(error instanceof Error ? error.message : it ? "Impossibile spostarlo nella collezione" : "Couldn't move this item to your collection") } }} />)}
      </div>
    </div>
  )
}

function WishlistRow({ entry: e, onRemove, onAcquire }: { entry: EnrichedWishlistItem; onRemove: () => void; onAcquire: () => void }) {
  const { locale, t } = useI18n(); const it = locale === "it"; const meta = PRIORITY_META[e.item.priority]
  return (
    <Card className="overflow-hidden py-0"><div className="flex items-stretch gap-3 p-3 sm:gap-4">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1.5">
        <div className="flex items-start justify-between gap-2"><div className="min-w-0"><Link href={`/catalog/${e.product.id}`} className="truncate font-medium hover:text-brand">{e.product.name}</Link><p className="truncate text-xs text-muted-foreground">{e.label ? `${e.label} · ` : `${t("wishlist.anyEdition")} · `}{e.product.chassis ?? "—"}</p></div><div className="flex items-center gap-1.5"><Badge className={meta.className}>{priorityLabel(e.item.priority, it)}</Badge><RarityBadge rarity={e.product.rarity} /></div></div>
        <div className="mt-auto flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1">{t("wishlist.market")} <span className="font-medium text-foreground">{formatMoney(e.estimate.value)}</span><TrendIndicator value={e.estimate.trend90d} className="text-xs" /></span>{e.item.targetPrice != null && <span className="inline-flex items-center gap-1"><Target className="size-3" /> {t("wishlist.target")} <span className="font-medium text-foreground">{formatMoney(e.item.targetPrice)}</span></span>}{e.belowTarget && <Badge className="bg-success text-white"><Check data-icon="inline-start" />{t("wishlist.atTarget")}</Badge>}</div>
      </div>
