...
1. **`lib/images/resolve.ts`** — pure functions implementing the priority
   ...
   e.g. Dash-1 Emperor's original 1990 box art shows even when its
   Premium/Anniversary siblings have their own (or no) images, because the
   release passed in is checked first.
...
This mattered in practice: several of this catalog's seed item numbers do
**not** match Tamiya's real numbering for that model — the seed's own
header comment already says as much ("item numbers... reflect real
releases as closely as possible... NOT a complete database"). For
example, this catalog's item `18626` is labeled "Aero Avante," but that
item number is a real, different Tamiya kit (Avante Mk.III Azure); the
real Aero Avante is Tamiya item `18701`. Where this was caught, the
mapping still keys on *this catalog's* identifier (so the image attaches
to the right row in *our* database) but records the real Tamiya item
actually used, and where a mismatch like this couldn't be confidently
resolved by name, the entry was left out entirely rather than risk
