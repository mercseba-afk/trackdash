...
| 0006 | `catalog_integrity_corrections` | Corrects already-deployed catalog rows in place (no schema table changes besides making `product_releases.item_number` nullable): wrong item numbers/chassis fixed against verified official Tamiya sources, fabricated pseudo-JAN and unverified/estimated MSRP cleared to `NULL`. Preserves every product/release primary key. See `docs/CATALOG_AUDIT.md`. |
| 0007 | `seed_catalog_images` | Initial real (official Tamiya, remote-hotlinked) images for a subset of the corrected catalog — `product_images`/`release_images` rows only, no schema change. Generated from `scripts/data/tamiya-images.ts` by `scripts/seed-images.mjs`. See `docs/IMAGES_MVP.md`. |
