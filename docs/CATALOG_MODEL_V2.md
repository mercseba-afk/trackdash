...
> **Deploy status:** migrations `0006`–`0010` were applied successfully
> to the live Supabase database on **5 September 2026**. See "Migration
> sequence" near the end for the per-migration detail.
...
TrackDash UUID. Identity derivation is:

- **Product UUID** ← the product's immutable `seedKey`
  (`stableUuid(product:${seedKey})`).
- **Release UUID** ← the immutable pair `productSeedKey +
  releaseSeedKey` (`stableUuid(release:${productSeedKey}:${releaseSeedKey})`).

Neither the Tamiya **item number** nor the release's **array position**
participates in identity: item number is correctable factual data, and
`releaseSeedKey` is an explicit, permanent per-release key (never
`i + 1`) — reordering a product's `releases` array leaves every UUID
byte-identical (see the reorder test in "Automated invariants"). The
same manufacturer number may
...
`scripts/check-catalog-invariants.mjs` surfaces it.
...
## Migration sequence (applied to live Supabase — 5 September 2026)
...
| `0009_release_sources_read_grants.sql` | Grants **only** `SELECT` on `release_sources` to `anon`/`authenticated` — the table's public-read RLS policy existed but the table-level SELECT privilege was missing. RLS stays enabled; NO insert/update/delete is granted, so catalog mutation remains admin-only. |
| `0010_catalog_trigger_search_path_hardening.sql` | Pins `search_path = public, pg_temp` on the three canonical-release trigger functions, resolving the Supabase security advisor `function_search_path_mutable`. Trigger behavior is unchanged — only name resolution is fixed. |

**Deployment status.** All five migrations
(`0006_catalog_model_v2`, `0007_catalog_normalization`,
`0008_seed_catalog_images`, `0009_release_sources_read_grants`,
`0010_catalog_trigger_search_path_hardening`) were applied successfully
to the live Supabase database on **5 September 2026**. `0009` and `0010`
were the only additional migrations the live deploy required (a missing
read grant and a security-advisor search_path fix); they are recorded in
the Drizzle journal after `0008` and introduce no new columns or tables,
so their schema snapshots are identical to `0008`'s.
